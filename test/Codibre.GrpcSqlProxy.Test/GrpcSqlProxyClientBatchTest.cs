...
public class GrpcSqlProxyClientBatchTest
{
    ...
    public async Task Should_Deal_With_Parameter_Limitation()
    {
        ...
        await channel.Batch.RunInTransaction(async (x) =>
        {
            x.AddNoResultScript($"DELETE FROM TB_PEDIDO");
            foreach (var i in pars)
            {
                await x.AddTransactionScript($"INSERT INTO TB_PEDIDO VALUES ({i})");
            }
            await x.FlushTransaction();
            await channel.Batch.PrepareEnumerable(pars, (i, b) =>
            {
